...
class AuthService
{
     ...
     */
    private function validateMockToken($token)
    {
        // Extract timestamp from token
        $timestamp = str_replace('test-token-', '', $token);

        // Generate a mock user based on timestamp
        $userId = (int)substr($timestamp, -6); // Last 6 digits as user ID
        $userName = 'TestUser' . substr($timestamp, -3); // Last 3 digits for name
