<?php
/**
 * Business Logo Upload Endpoint for Interviews.tv API
...
try {
    ...
    if (!isset($_FILES['business_logo'])) {
        ApiResponse::error('No business logo file provided', 400);
    }
    ...
    // Validate file size (5MB max for business logos)
    ...
    $uploadDir = '../storage/uploads/business-logos';
    if (!is_dir($uploadDir)) {
    ...
    $filename = "logo_{$currentUser['id']}_" . time() . ".{$extension}";
    ...
    // Return the upload result (business logos are not stored in user table)
    $logoUrl = "/api/uploads/business-logos/{$filename}";
    ...
    ApiResponse::error('Business logo upload failed: ' . $e->getMessage(), 500);
}
